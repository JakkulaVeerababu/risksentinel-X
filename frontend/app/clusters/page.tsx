...
const entityNodes = [
  { label: "DEV-8812", tag: "DEV", x: "50%", y: "44%", tone: "border-[#255df5] bg-[#255df5] text-white", size: "h-14 w-14" },
  { label: "A. Sharma", tag: "ID", x: "21%", y: "23%", tone: "border-[#aeb9ca] bg-white text-[#334158]", size: "h-11 w-11" },
  { label: "M. Rao", tag: "ID", x: "78%", y: "20%", tone: "border-[#aeb9ca] bg-white text-[#334158]", size: "h-11 w-11" },
  { label: "49.37.18.2", tag: "IP", x: "18%", y: "72%", tone: "border-[#7558c9] bg-white text-[#6748b8]", size: "h-11 w-11" },
  { label: "Card •• 8219", tag: "PAY", x: "82%", y: "70%", tone: "border-[#c98925] bg-white text-[#a55f00]", size: "h-11 w-11" },
];
      ...
      <section className="grid min-h-[620px] gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
        ...
        <div className="rsx-data-panel">
          <div className="flex flex-col gap-3 border-b border-[#e6ebf3] bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button onClick={() => setContained((items) => items.includes(selected.id) ? items : [...items, selected.id])} className={`inline-flex h-9 items-center justify-center rounded-lg border px-3.5 text-[10px] font-bold transition-colors opacity-75 cursor-not-allowed ${contained.includes(selected.id) ? "border-[#315efb] bg-[#edf3ff] text-[#255df5]" : "border-[#315efb] bg-[#315efb] text-white"}`} disabled>{contained.includes(selected.id) ? "Cluster contained" : "Contain cluster (Demo)"}</button>
          </div>
          ...
          <div className="grid lg:grid-cols-[minmax(0,1fr)_272px]">
            <div className="rsx-grid relative min-h-[390px] overflow-hidden border-b border-[#e6ebf3] bg-[#fbfcff] lg:border-b-0 lg:border-r">
              ...
              {entityNodes.map((node) => <div key={node.label} className="absolute -translate-x-1/2 -translate-y-1/2 text-center" style={{ left: node.x, top: node.y }}><span className={`mx-auto flex ${node.size} items-center justify-center rounded-full border-2 ring-4 ring-white ${node.tone}`}><span className="font-mono text-[9px] font-bold tracking-[.04em]">{node.tag}</span></span><span className="mt-2 block rounded-md border border-[#e4e9f1] bg-white/95 px-1.5 py-0.5 font-mono text-[10px] font-bold text-[#344158]">{node.label}</span></div>)}
