...
import { ArrowLeft, ArrowRight, Clock, History, AlertTriangle, Search, Filter, CheckCircle2 } from "lucide-react";
      ...
      <div className="bg-surface border border-border p-8 rounded-2xl shadow-sm premium-shadow flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-heading-lg text-mono-sm font-mono font-semibold text-primary ">{inv.case_id}</h1>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-surface border border-border rounded-xl p-6 shadow-sm relative overflow-hidden group">
            ...
            <h3 className="rsx-rule-heading mb-5">Summary</h3>
            <p className="text-text-secondary text-label-sm leading-relaxed max-w-3xl">
              This case was auto-generated by the Risk Policy Engine due to a high-risk event trigger: <strong className="text-text-primary bg-surface-secondary px-2 py-1 rounded-md border border-border text-mono-sm font-mono">{inv.trigger}</strong>.
              The total financial exposure associated with the linked transactions is <strong className="text-danger text-mono-sm font-mono font-semibold tabular-nums bg-danger-soft px-2 py-1 rounded-md border border-danger/20">₹{(inv.exposure || 0).toLocaleString()}</strong>.
            </p>
          </div>
            ...
            <div className="overflow-y-auto p-5 space-y-3 bg-surface-secondary/30">
              {inv.transactions && inv.transactions.map((txId: string) => (
                <div key={txId} className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-4 bg-surface border border-border rounded-xl hover:border-primary/30 transition-colors shadow-sm gap-3">
                  <div className="text-mono-sm font-mono text-label-sm text-primary font-semibold">{txId}</div>
          ...
          <div className="bg-surface border border-border rounded-xl p-6 shadow-sm relative overflow-hidden group hover:border-primary/50 transition-colors cursor-pointer">
            <h3 className="text-caption font-semibold text-text-primary uppercase mb-5 flex items-center justify-between">
              <span className="rsx-rule-heading">Graph context</span>
            ...
            <div className="h-56 bg-surface-secondary border border-border rounded-xl flex items-center justify-center relative overflow-hidden group-hover:bg-primary-soft/10 transition-colors">
              ...
              <div className="z-10 border border-border bg-surface/90 p-6 text-center backdrop-blur-sm">
                <div className="mb-1 text-label-sm font-semibold text-text-primary">Neighborhood analysis ready</div>
          ...
          <div className="bg-surface border border-border rounded-xl p-6 shadow-sm premium-shadow relative overflow-hidden">
            ...
            <div className="space-y-3 relative z-10">
              <button
                ...
                className="w-full bg-success hover:bg-[#315efb] text-white font-semibold text-label-sm py-3 rounded-xl transition-all shadow-sm flex items-center justify-center gap-2"
              >
                ...
                <div className="relative">
                  ...
                  <div className="pl-5">
                    ...
                    <div className="text-caption text-text-secondary mt-3 bg-surface border border-border p-3.5 rounded-xl shadow-sm leading-relaxed">
                      Policy Engine triggered creation due to <strong className="text-text-primary text-mono-sm font-mono bg-surface-secondary px-1.5 py-0.5 rounded border border-border/50 ml-1">{inv.trigger}</strong>
                    </div>
                  </div>
                </div>
