  ...
  return (
    <div className="rsx-page space-y-5">
      ...
      <section className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        ...
        ].map((metric) => <div key={metric.label} className="rsx-card p-4 sm:p-5"><p className="text-[10px] font-bold text-[#7c8799]">{metric.label}</p><div className="mt-2 flex items-end justify-between gap-2"><p className="text-[23px] font-bold tracking-[-.04em] text-[#17233f] sm:text-[27px]">{metric.value}</p><span className={`mb-1 flex h-6 w-6 items-center justify-center rounded-full ${metric.positive ? "bg-[#eafaf3] text-[#07845a]" : "bg-[#fff6e9] text-[#b96800]"}`}>{metric.positive ? <ArrowUpRight className="h-3.5 w-3.5" /> : <ArrowDownRight className="h-3.5 w-3.5" />}</span></div><p className={`mt-1 text-[9px] font-semibold ${metric.positive ? "text-[#07845a]" : "text-[#b96800]"}`}>{metric.helper}</p></div>)}
      </section>

      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.75fr)]">
        <div className="rsx-card overflow-hidden">
          <div className="flex flex-col gap-3 border-b border-[#edf0f5] px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-[13px] font-bold text-[#253149]">Decision performance</h2><p className="mt-0.5 text-[9px] text-[#8b95a7]">Processed volume with risky and blocked overlays</p></div><div className="flex items-center gap-1 rounded-lg bg-[#f3f5f8] p-0.5">{["24H","7D","30D"].map((item) => <button key={item} onClick={() => setPeriod(item)} className={`rounded-md px-2.5 py-1.5 text-[9px] font-extrabold ${period === item ? "bg-white text-[#255df5] shadow-sm" : "text-[#7f899a]"}`}>{item}</button>)}</div></div>
          ...
          <div className="grid grid-cols-3 border-t border-[#edf0f5] bg-[#fafbfd]">{[["Analysed", "157.3K"], ["High risk", "3,627"], ["Blocked", "916"]].map(([label,value]) => <div key={label} className="border-r border-[#edf0f5] px-4 py-3 last:border-r-0"><p className="text-[8px] font-bold uppercase tracking-wider text-[#909aab]">{label}</p><p className="mt-1 text-[14px] font-bold text-[#344158]">{value}</p></div>)}</div>
        </div>

        <div className="rsx-card overflow-hidden">
          <div className="border-b border-[#edf0f5] px-5 py-4"><h2 className="text-[13px] font-bold text-[#253149]">Risk signal mix</h2><p className="mt-0.5 text-[9px] text-[#8b95a7]">Share of high-risk decisions</p></div>
          <div className="relative h-[220px] p-3"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={reasons} dataKey="value" innerRadius={57} outerRadius={82} paddingAngle={3} stroke="none">{reasons.map((reason) => <Cell key={reason.name} fill={reason.color}/>)}</Pie><Tooltip contentStyle={{ borderRadius: 10, borderColor: "#e1e6ee", fontSize: 10 }}/></PieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center"><span className="text-[22px] font-bold tracking-[-.04em] text-[#17233f]">3,627</span><span className="text-[8px] font-bold uppercase tracking-wider text-[#8b95a7]">signals</span></div></div>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <div className="rsx-card overflow-hidden"><div className="flex items-center justify-between border-b border-[#edf0f5] px-5 py-4"><div><h2 className="text-[13px] font-bold">Risk rate by hour</h2><p className="mt-0.5 text-[9px] text-[#8b95a7]">High-risk share of analysed payments</p></div><button className="rounded-lg border border-[#e1e6ee] p-2 text-[#7c8799]"><Filter className="h-4 w-4" /></button></div><div className="h-[250px] p-5"><ResponsiveContainer width="100%" height="100%"><BarChart data={hourly}><CartesianGrid vertical={false} stroke="#edf0f5"/><XAxis dataKey="hour" tickLine={false} axisLine={false} tick={{fontSize:9,fill:"#8a94a6"}}/><YAxis tickLine={false} axisLine={false} tick={{fontSize:9,fill:"#8a94a6"}} tickFormatter={(value)=>`${value}%`}/><Tooltip contentStyle={{borderRadius:10,borderColor:"#e1e6ee",fontSize:10}}/><Bar dataKey="rate" name="Risk rate" fill="#7446d8" radius={[5,5,0,0]} maxBarSize={28}/></BarChart></ResponsiveContainer></div></div>
        <div className="overflow-hidden rounded-2xl border border-[#253251] bg-[#10172a] p-5 text-white shadow-[0_14px_35px_rgba(16,23,42,.12)]"><div className="flex items-center justify-between"><div><p className="text-[9px] font-black uppercase tracking-[.15em] text-[#8799bb]">Decision health</p><h2 className="mt-1.5 text-[16px] font-bold">Controls are performing within target</h2></div><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#143c39] text-[#49d4a4]"><ShieldCheck className="h-5 w-5" /></span></div><div className="mt-6 space-y-4">{[["ML precision", "93.8%", 94, "#5f8cff"], ["Policy coverage", "98.4%", 98, "#42c996"], ["Graph enrichment", "87.2%", 87, "#9b72f7"]].map(([label,value,width,color]) => <div key={String(label)}><div className="flex justify-between text-[10px] font-semibold"><span className="text-[#aab7cf]">{label}</span><span>{value}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full" style={{width:`${width}%`,background:String(color)}}/></div></div>)}</div><p className="mt-6 rounded-xl border border-white/10 bg-white/5 p-3 text-[10px] leading-5 text-[#aebbd2]">False positives fell for the third consecutive day while protected value increased by 16.8%.</p></div>
      </section>

      {notice && <div className="fixed bottom-5 right-5 z-[120] flex items-center gap-2 rounded-xl border border-[#cdebdc] bg-white px-4 py-3 text-[11px] font-bold text-[#087a50] shadow-[0_16px_40px_rgba(16,24,40,.15)]"><ShieldCheck className="h-4 w-4" /> Analytics report exported</div>}
    </div>
  );
}
