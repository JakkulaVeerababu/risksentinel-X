...
import { fetchSettings, updateSettings, fetchTeam, addTeamMember } from "@/lib/api";
...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: () => void; label: string }) {
  return <div className="flex shrink-0 items-center gap-2.5"><span aria-hidden="true" className="w-5 text-[11px] text-[#7b8799]">{checked ? "On" : "Off"}</span><button type="button" role="switch" aria-checked={checked} aria-label={label} onClick={onChange} className="settings-switch"><span aria-hidden="true" className="settings-switch-thumb" /></button></div>;
}
...
export default function SettingsPage() {
      ...
      try {
        ...
        if (s) {
          setSettings({
            ...
            graph_enrichment: s.graph_enrichment,
            ai_reasoning: s.ai_reasoning,
            step_up: s.step_up,
            email_critical: s.email_critical,
  ...
  const handleInputChange = (key: keyof typeof settings, value: any) => setSettings((state) => ({ ...state, [key]: value }));
  ...
  const inviteMember = async () => {
    try {
      ...
      const res = await addTeamMember({ name, email, role: "Fraud analyst" });
      setTeam([...team, res]);
    } catch (e) {
      console.error("Failed to invite member:", e);
      alert("Failed to invite member. Maybe email already exists.");
    }
  };

  if (loading) return <div className="p-10 text-center text-text-muted">Loading settings...</div>;

  return (
    <div className="rsx-page space-y-5">
      <PageHeader eyebrow="Administration" title="Settings" description="Configure workspace defaults, detection behavior, integrations, notifications, and team access." actions={<button onClick={save} className="inline-flex h-9 items-center gap-2 rounded-lg bg-[#255df5] px-3.5 text-[11px] font-bold text-white hover:bg-[#1a4acc]"><Save className="h-4 w-4" /> Save changes</button>} />
      ...
      <section className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
        ...
        <div className="space-y-4">
          {activeTab === "workspace" && <>
            <div className="rsx-card overflow-hidden"><div className="border-b border-[#edf0f5] px-5 py-4"><h2 className="text-[13px] font-bold">Workspace profile</h2><p className="mt-0.5 text-[10px] text-[#8b95a7]">Displayed across reports and audit records</p></div><div className="grid gap-4 p-5 sm:grid-cols-2"><label className="text-[10px] font-bold text-[#5f6b80]">Workspace name<input value={settings.workspace_name} onChange={(e) => handleInputChange("workspace_name", e.target.value)} className="mt-2 h-10 w-full rounded-lg border border-[#dfe5ee] bg-white px-3 text-[11px] font-medium text-[#27334b] outline-none focus:border-[#7fa6ff]" /></label><label className="text-[10px] font-bold text-[#5f6b80]">Merchant ID<input value="K8p2xY14" readOnly className="mt-2 h-10 w-full rounded-lg border border-[#e1e6ee] bg-[#f7f9fc] px-3 font-mono text-[11px] text-[#7a8597]" /></label><label className="text-[10px] font-bold text-[#5f6b80]">Default currency<select value={settings.currency} onChange={(e) => handleInputChange("currency", e.target.value)} className="mt-2 h-10 w-full rounded-lg border border-[#dfe5ee] bg-white px-3 text-[11px] text-[#27334b]"><option value="INR">INR</option><option value="USD">USD</option><option value="EUR">EUR</option></select></label><label className="text-[10px] font-bold text-[#5f6b80]">Timezone<select value={settings.timezone} onChange={(e) => handleInputChange("timezone", e.target.value)} className="mt-2 h-10 w-full rounded-lg border border-[#dfe5ee] bg-white px-3 text-[11px] text-[#27334b]"><option value="Asia/Kolkata">Asia/Kolkata</option><option value="UTC">UTC</option><option value="America/New_York">America/New_York</option></select></label></div></div>
          ...
          {activeTab === "detection" && <div className="rsx-card overflow-hidden"><div className="border-b border-[#edf0f5] px-5 py-4"><h2 className="text-[13px] font-bold">Decision orchestration</h2><p className="mt-0.5 text-[10px] text-[#8b95a7]">Controls applied to payment decisions</p></div><div className="divide-y divide-[#edf0f5]">{[
            { key: "auto_block" as const, title: "Automatic blocking", detail: "Apply BLOCK decisions when policy and confidence thresholds are satisfied", badge: "Recommended" },
            { key: "graph_enrichment" as const, title: "Graph enrichment", detail: "Evaluate shared devices, identities, instruments, and network clusters", badge: "Active" },
            { key: "ai_reasoning" as const, title: "Intelligence explanations", detail: "Generate plain-language evidence summaries for analyst review", badge: "Agentic" },
            { key: "step_up" as const, title: "Trusted-customer step-up", detail: "Prefer additional verification over blocking when trust signals are strong", badge: "Conversion" },
          ].map((item) => <div key={item.key} className="flex items-center justify-between gap-4 p-5"><div><div className="flex flex-wrap items-center gap-2"><p className="text-[11px] font-bold text-[#344158]">{item.title}</p><span className="rounded-full bg-[#edf3ff] px-2 py-0.5 text-[10px] font-extrabold text-[#255df5]">{item.badge}</span></div><p className="mt-1 text-[10px] leading-4 text-[#8791a3]">{item.detail}</p></div><Toggle label={item.title} checked={settings[item.key]} onChange={() => toggle(item.key)} /></div>)}</div><div className="border-t border-[#edf0f5] bg-[#fafbfd] p-5"><label className="text-[10px] font-bold text-[#5f6b80]">Global review threshold <span className="ml-2 text-[#255df5]">{settings.review_threshold}</span><input type="range" min="0.1" max="0.95" step="0.01" value={settings.review_threshold} onChange={(e) => handleInputChange("review_threshold", parseFloat(e.target.value))} className="mt-3 block w-full accent-[#255df5]" /></label></div></div>}
          ...
          {activeTab === "notifications" && <div className="rsx-card overflow-hidden"><div className="border-b border-[#edf0f5] px-5 py-4"><h2 className="text-[13px] font-bold">Notification rules</h2><p className="mt-0.5 text-[10px] text-[#8b95a7]">Choose when the Fraud Ops team should be notified</p></div><div className="divide-y divide-[#edf0f5]">{[
            { key: "email_critical" as const, title: "Critical fraud clusters", detail: "Instant email when a new critical cluster is detected" },
            { key: "email_summary" as const, title: "Daily risk summary", detail: "A concise 9:00 AM overview of decisions and exposure" },
            { key: "slack_alerts" as const, title: "Slack alerts", detail: "Send urgent events to #fraud-operations" },
          ...
          {activeTab === "access" && <>
            <div className="rsx-card overflow-hidden"><div className="flex items-center justify-between border-b border-[#edf0f5] px-5 py-4"><div><h2 className="text-[13px] font-bold">Team access</h2><p className="mt-0.5 text-[10px] text-[#8b95a7]">{team.length} members · role-based permissions</p></div><button onClick={inviteMember} className="rounded-lg bg-[#255df5] px-3 py-2 text-[10px] font-bold text-white hover:bg-[#1a4acc]">Invite member</button></div><div className="divide-y divide-[#edf0f5]">{team.map((member) => <div key={member.email} className="flex items-center gap-3 p-4 sm:px-5"><div className="min-w-0 flex-1"><p className="text-[11px] font-bold text-[#344158]">{member.name}</p><p className="truncate text-[10px] text-[#8791a3]">{member.email}</p></div><button className="rounded-lg border border-[#dfe5ee] px-3 py-2 text-[10px] font-bold text-[#536078]">{member.role}</button></div>)}</div></div>
