  ...
  return (
    <>
      ...
      {showCriticalAlert && metricsData.alert && (
        <section className="relative mb-6 overflow-hidden rounded-lg border border-danger/30 bg-danger-soft px-5 py-5 text-danger shadow-sm sm:px-6">
          <div className="relative flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-3">
                <p className="text-[10px] font-bold uppercase tracking-[0.17em] text-danger">Priority intelligence</p>
                <span className="rounded-md border border-danger/40 bg-danger/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-danger">Critical</span>
              </div>
              <h3 className="mt-2 text-[18px] font-semibold text-danger">Coordinated cluster FRC-0184 requires review</h3>
              <p className="mt-1 text-[13px] leading-6 text-danger">
                {metricsData.alert.transaction_count} transactions share devices. The policy engine recommends blocking the linked cluster.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-6 xl:justify-end">
              <Link href="/cases" className="flex items-center gap-2 rounded-lg bg-danger px-4 py-2 text-[12px] font-semibold text-white transition hover:bg-danger/90">
                Investigate <ExternalLink className="h-3.5 w-3.5" />
              </Link>
              <button onClick={() => setShowCriticalAlert(false)} className="rounded-md p-2 text-danger/70 hover:bg-danger/10 hover:text-danger" aria-label="Dismiss">
                <X className="h-4 w-4" />
              </button>
            </div>
          </div>
      ...
      <section className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {metrics.map((metric) => {
          return (
            <article key={metric.label} className="group relative flex flex-col gap-2 rounded-lg border border-border bg-surface p-5 shadow-sm">
      ...
      <section className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-12">
        <article className="rounded-xl border border-border bg-surface shadow-sm lg:col-span-8 overflow-hidden">
        ...
        <article className="rounded-xl border border-border bg-surface p-6 shadow-sm lg:col-span-4 flex flex-col justify-between">
          <div>
      ...
      <section className="grid grid-cols-1 gap-6 lg:col-span-12 pb-12">
            ...
            <table className="w-full min-w-[760px] border-collapse text-left">
              <thead>
                <tr className="border-b border-border bg-surface-secondary text-caption font-semibold uppercase text-text-muted">
                  ...
                  <th className="px-4 py-3.5 hidden md:table-cell">ML Risk</th>
                  <th className="px-4 py-3.5 hidden md:table-cell">Graph Risk</th>
                  <th className="px-4 py-3.5">Decision</th>
                  <th className="px-5 py-3.5 text-right hidden lg:table-cell">Detected</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {transactionsData.slice(0, 10).map((t) => (
                  <tr key={t.transaction_id} className="transition-colors hover:bg-surface-secondary/50 group">
                    ...
                    <td className="px-4 py-4 text-label-sm font-semibold text-text-primary tabular-nums">₹{t.amount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    ...
                    <td className="px-5 py-4 text-right text-caption text-text-muted text-mono-sm font-mono hidden lg:table-cell">{t.timestamp ? timeAgo(t.timestamp) : "Just now"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
