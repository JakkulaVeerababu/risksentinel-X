...
- **Built an event-driven risk backend** with FastAPI and PostgreSQL, implementing Server-Sent Events (SSE) to stream real-time investigation telemetry to a Next.js live dashboard.
- **Designed a deterministic Policy Engine** that safely wraps probabilistic ML and LLM outputs, ensuring repeatable ALLOW/REVIEW/BLOCK decisions and immutable auditability.
