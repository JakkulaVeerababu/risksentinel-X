...
print("===== TEST 13: AGENT BLOCK 0.99 + WEAK GRAPH =====")

# Step 10: Confidence Extremes
print("\n--- Confidence Regression ---")
for conf in [0.50, 0.90, 0.99, 1.00]:
    res = policy_engine.evaluate(PolicyInput(
        transaction_id=f"CONF_{conf}", ml_score=0.919, ml_model_version="xgb-v1",
        graph_score=0.088, graph_version="v2",
        agent_state="COMPLETED", agent_recommendation="BLOCK", agent_confidence=conf
    ))
    print(f"BLOCK @ {conf:.2f} confidence -> Decision: {res.final_decision}, Rule: {res.matched_rule_ids}")

# Step 11: Graph Corroboration Contrast
print("\n--- Corroboration Contrast ---")
res_weak = policy_engine.evaluate(PolicyInput(
    transaction_id="CONT_WEAK", ml_score=0.919, ml_model_version="xgb-v1",
    graph_score=0.088, graph_version="v2",
    agent_state="COMPLETED", agent_recommendation="BLOCK", agent_confidence=0.99
))
res_strong = policy_engine.evaluate(PolicyInput(
    transaction_id="CONT_STRONG", ml_score=0.919, ml_model_version="xgb-v1",
    graph_score=1.0, graph_version="v2",
    agent_state="COMPLETED", agent_recommendation="BLOCK", agent_confidence=0.99
))
print(f"High ML + LOW graph + Agent BLOCK 0.99 -> Decision: {res_weak.final_decision}")
print(f"High ML + HIGH graph + Agent BLOCK 0.99 -> Decision: {res_strong.final_decision}")

# Step 12: Low/Low Policy Safety
print("\n--- LOW/LOW Policy Safety ---")
res_lowlow = policy_engine.evaluate(PolicyInput(
    transaction_id="LOW_LOW", ml_score=0.321, ml_model_version="xgb-v1",
    graph_score=0.088, graph_version="v2",
    agent_state="COMPLETED", agent_recommendation="BLOCK", agent_confidence=0.99
))
print(f"Low ML + Low graph + Agent BLOCK 0.99 -> Decision: {res_lowlow.final_decision}, Rule: {res_lowlow.matched_rule_ids}")
...
payload = {
    ...
    "customer_id": "CUST_13",
    "entity_id": "C0025" # Low risk cluster, graph_risk ~ 0.088
}
...
def get_mock_investigation(*args, **kwargs):
    print("MOCK CALLED")
    return InvestigationResult(
        recommendation="BLOCK",
        confidence=0.99,
        reason_codes=["HIGH_ML_RISK"],
        evidence=[]
    )

# Directly patch the class method
original_ollama = OllamaProvider.generate_structured_investigation
OllamaProvider.generate_structured_investigation = get_mock_investigation

original_mock = MockProvider.generate_structured_investigation
MockProvider.generate_structured_investigation = get_mock_investigation
